...
const PaperIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>;
...
const navItems = [
    ...
    { path: '/admin/papers', name: 'Past Papers', icon: <PaperIcon /> },
    { path: '/admin/guides', name: 'Study Guides', icon: <GuideIcon /> },
];

const getLocationTitle = (pathname: string) => {
    if (pathname.includes('/users')) return 'Manage Users';
    if (pathname.includes('/papers')) return 'Manage Papers';
    if (pathname.includes('/guides')) return 'Manage Guides';
                ...
                <nav className="flex-1 space-y-2">
                    {navItems.map(item => (
                        <NavLink
                            ...
                            onClick={onClose}
                            className={({ isActive }) =>
                                `flex items-center space-x-3 px-4 py-3 rounded-lg transition-colors duration-200 ${isActive ? 'bg-primary/80 text-white font-semibold' : 'text-slate-300 hover:bg-slate-700'
                                }`
                            }
                        >
                            {item.icon}
