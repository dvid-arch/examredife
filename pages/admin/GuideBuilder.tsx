...
const GuideBuilder: React.FC = () => {
    ...
    return (
        <div className="space-y-4 h-[calc(100vh-100px)] flex flex-col md:overflow-hidden">
            {/* Header */}
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shrink-0">
                <div>
                    <button onClick={() => navigate('/admin/guides')} className="flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-slate-100 transition-colors mb-2">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M9.707 16.707a1 1 0 01-1.414 0l-6-6a1 1 0 010-1.414l6-6a1 1 0 011.414 1.414L5.414 9H17a1 1 0 110 2H5.414l4.293 4.293a1 1 0 010 1.414z" clipRule="evenodd" /></svg>
                        Back to Guides
                    </button>
                    <h1 className="text-2xl font-bold text-slate-800 dark:text-white flex items-center gap-3">
                        Guide Builder
                        <span className="text-sm font-normal px-2 py-0.5 rounded-md bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300">
                            {guide.subject}
                        </span>
                    </h1>
                </div>
            ...
            <div className="flex-1 flex flex-col md:flex-row gap-4 min-h-0">
                ...
                <Card className="w-full md:w-72 flex flex-col shrink-0 p-0 overflow-hidden border-t-4 border-t-indigo-500">
                    ...
                    <div className="flex-1 overflow-y-auto p-2 space-y-1 bg-slate-50/50 dark:bg-slate-800/20">
                ...
                <Card className="flex-1 flex flex-col min-w-0 p-0 overflow-hidden shadow-lg border-t-4 border-t-primary">
                    {activeTopic ? (
                        <>
                            <div className="p-4 border-b dark:border-slate-700 bg-white dark:bg-slate-800 shrink-0">
                                ...
                                <input
                                    ...
                                    className="w-full text-xl font-bold p-2 bg-transparent border-b-2 border-transparent hover:border-slate-200 dark:hover:border-slate-700 focus:border-primary dark:focus:border-primary transition-colors outline-none text-slate-800 dark:text-slate-100"
                                    placeholder="Enter topic title..."
                                />
                            </div>
                            <div className="flex-1 flex flex-col md:flex-row min-h-0 bg-slate-50 dark:bg-slate-900">
                                {/* Raw Markdown Editor */}
                                <div className="flex-1 flex flex-col border-r dark:border-slate-700">
                                    <div className="p-2 bg-slate-100 dark:bg-slate-800 text-xs font-bold text-slate-500 uppercase flex justify-between items-center">
                                        ...
                                        <span className="text-[10px] font-normal italic">Use standard Markdown syntax</span>
                                    </div>
                                    <textarea
                        ...
                        <div className="flex-1 flex items-center justify-center text-slate-500 bg-slate-50 dark:bg-slate-800/50">
