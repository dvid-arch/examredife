...
const GuideReader: React.FC = () => {
    ...
    useEffect(() => {
        if (guide?.content) {
            ...
            const headers = guide.content.match(/^#{2,3}\s+(.+)$/gm);
            if (headers) {
    ...
    return (
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8 pb-20">
            {/* Sidebar TOC */}
            <aside className="lg:w-64 flex-shrink-0 order-2 lg:order-1 hidden lg:block">
            ...
            <div className="flex-1 min-w-0 order-1 lg:order-2 space-y-6">
                ...
                <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-xl overflow-hidden border border-slate-100 dark:border-slate-800">
                    <div className="p-8 border-b border-slate-100 dark:border-slate-800 bg-gradient-to-r from-slate-50 to-white dark:from-slate-800/50 dark:to-slate-900">
                        <h1 className="text-3xl sm:text-4xl font-extrabold text-slate-800 dark:text-white mb-2 leading-tight">
                            {guide.title}
                        </h1>
